...
import { EntityArrayResponseType, JobsService } from '../service/jobs.service';
...
export class JobsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IJobs): number => this.jobsService.getJobsIdentifier(item);

  search(query: string): void {
    if (query && JobsComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (JobsComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IJobs[] | null): IJobs[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.jobsService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.jobsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
