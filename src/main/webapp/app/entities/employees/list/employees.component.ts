...
import { EntityArrayResponseType, EmployeesService } from '../service/employees.service';
...
export class EmployeesComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEmployees): number => this.employeesService.getEmployeesIdentifier(item);

  search(query: string): void {
    if (query && EmployeesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (EmployeesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IEmployees[] | null): IEmployees[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.employeesService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.employeesService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
