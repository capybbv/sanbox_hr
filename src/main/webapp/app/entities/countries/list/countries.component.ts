...
export class CountriesComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICountries): number => this.countriesService.getCountriesIdentifier(item);

  search(query: string): void {
    if (query && CountriesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CountriesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ICountries[] | null): ICountries[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.countriesService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
