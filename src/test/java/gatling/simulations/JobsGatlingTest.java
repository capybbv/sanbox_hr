...
/**
 * Performance test for the Jobs entity.
 */
public class JobsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(http("Get all jobs").get("/api/jobs").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new jobs")
                        .post("/api/jobs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"jobId\": 0" + ", \"jobTitle\": \"SAMPLE_TEXT\"" + ", \"minSalary\": 0" + ", \"maxSalary\": 0" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_jobs_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created jobs").get("${new_jobs_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created jobs").delete("${new_jobs_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Jobs entity").exec(scn);
