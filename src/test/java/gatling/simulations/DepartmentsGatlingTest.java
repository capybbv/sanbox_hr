 ...
 */
public class DepartmentsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(http("Get all departments").get("/api/departments").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new departments")
                        .post("/api/departments")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"departmentId\": 0" + ", \"departmentName\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_departments_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created departments").get("${new_departments_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created departments").delete("${new_departments_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Departments entity").exec(scn);
