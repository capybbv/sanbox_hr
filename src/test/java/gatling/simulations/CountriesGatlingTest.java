 ...
 */
public class CountriesGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(http("Get all countries").get("/api/countries").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new countries")
                        .post("/api/countries")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"countryId\": \"SAMPLE_TEXT\"" + ", \"countryName\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_countries_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created countries").get("${new_countries_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created countries").delete("${new_countries_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Countries entity").exec(scn);
