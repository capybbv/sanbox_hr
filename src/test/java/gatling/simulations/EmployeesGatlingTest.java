 ...
 */
public class EmployeesGatlingTest extends Simulation {
            ...
            exec(http("Get all employees").get("/api/employees").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new employees")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_employees_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created employees").get("${new_employees_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created employees").delete("${new_employees_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Employees entity").exec(scn);
