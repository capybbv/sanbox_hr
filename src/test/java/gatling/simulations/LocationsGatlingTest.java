 ...
 */
public class LocationsGatlingTest extends Simulation {
            ...
            exec(http("Get all locations").get("/api/locations").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new locations")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_locations_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created locations").get("${new_locations_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created locations").delete("${new_locations_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Locations entity").exec(scn);
